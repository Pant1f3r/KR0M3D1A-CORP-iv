  ...
  return (
    <div className="bg-cyber-surface/50 p-6 rounded-lg border border-cyber-primary/20 h-full flex flex-col">
      ...
      <div className="text-center mb-4">
        <div className="text-xs text-cyber-dim">PROTOCOL STATUS</div>
        <div className={`font-orbitron text-2xl font-bold ${statusInfo.color} flex items-center justify-center gap-2`}>
            <StatusIcon className={`w-6 h-6 ${status === 'AUTONOMOUS' ? 'w-8 h-8' : ''}`}/>
            <span>{statusInfo.label}</span>
        </div>
      </div>
      
      <div className="flex justify-around text-center mb-4 border-y border-cyber-primary/10 py-2">
        <div>
            ...
            <ul className="space-y-2">
              {patchLog.map(log => (
                <li key={log.id} className="text-xs font-roboto-mono flex items-start gap-2">
                  ...
                  <span className={log.status === 'APPLIED' ? 'text-cyber-accent' : 'text-red-500'}>{log.description}</span>
                </li>
              ))}
            </ul>
          ) : (
      ...
      <div className="mt-auto pt-4 border-t border-cyber-primary/20">
            <h4 className="font-orbitron text-cyber-accent">Vanguard Mandate</h4>
            <p className="text-xs text-cyber-dim font-roboto-mono mt-1">
